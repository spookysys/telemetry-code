...
#define LoggerDebug false  // set to true to get Serial output for debugging

#define HARD_CODED_CALIBRATION_VALUES
...
namespace imu {
  ...
  bool begin()
  {
    ...
    */
    ...
    logger.println("MPU9250 9-axis motion sensor...");
    byte c = readByte(MPU9250_ADDRESS, WHO_AM_I_MPU9250);  // Read WHO_AM_I register for MPU-9250
    logger.print("MPU9250 "); logger.print("I AM "); logger.print(c, HEX); logger.print(" I should be "); logger.println(0x71, HEX);
    ...
    */
    ...
    if (c != 0x71) return false; // WHO_AM_I should always be 0x68
 
    logger.println("MPU9250 is online...");

    MPU9250SelfTest(SelfTest); // Start by performing self test and reporting values
    logger.print("x-axis self test: acceleration trim within : "); logger.print(SelfTest[0], 1); logger.println("% of factory value");
    logger.print("y-axis self test: acceleration trim within : "); logger.print(SelfTest[1], 1); logger.println("% of factory value");
    logger.print("z-axis self test: acceleration trim within : "); logger.print(SelfTest[2], 1); logger.println("% of factory value");
    logger.print("x-axis self test: gyration trim within : "); logger.print(SelfTest[3], 1); logger.println("% of factory value");
    logger.print("y-axis self test: gyration trim within : "); logger.print(SelfTest[4], 1); logger.println("% of factory value");
    logger.print("z-axis self test: gyration trim within : "); logger.print(SelfTest[5], 1); logger.println("% of factory value");
    delay(1000);
    ...
    logger.println(" Calibrate gyro and accel");
    accelgyrocalMPU9250(gyroBias, accelBias); // Calibrate gyro and accelerometers, load biases in bias registers
    logger.println("accel biases (mg)"); logger.println(1000.*accelBias[0]); logger.println(1000.*accelBias[1]); logger.println(1000.*accelBias[2]);
    logger.println("gyro biases (dps)"); logger.println(gyroBias[0]); logger.println(gyroBias[1]); logger.println(gyroBias[2]);
    ...
    */
    ...
    initMPU9250();
    logger.println("MPU9250 initialized for active data mode...."); // Initialize device for active mode read of acclerometer, gyroscope, and temperature

    // Read the WHO_AM_I register of the magnetometer, this is a good test of communication
    byte d = readByte(AK8963_ADDRESS, WHO_AM_I_AK8963);  // Read WHO_AM_I register for AK8963
    logger.print("AK8963 "); logger.print("I AM "); logger.print(d, HEX); logger.print(" I should be "); logger.println(0x48, HEX);
    ...
    */
    ...
    initAK8963(magCalibration); logger.println("AK8963 initialized for active data mode...."); // Initialize device for active mode read of magnetometer

    magcalMPU9250(magBias, magScale);
    logger.println("AK8963 mag biases (mG)"); logger.println(magBias[0]); logger.println(magBias[1]); logger.println(magBias[2]);
    logger.println("AK8963 mag scale (mG)"); logger.println(magScale[0]); logger.println(magScale[1]); logger.println(magScale[2]);
    delay(2000); // add delay to see results before serial spew of data

    if (LoggerDebug) {
      //  logger.println("Calibration values: ");
      logger.print("X-Axis sensitivity adjustment value "); logger.println(magCalibration[0], 2);
      logger.print("Y-Axis sensitivity adjustment value "); logger.println(magCalibration[1], 2);
      logger.print("Z-Axis sensitivity adjustment value "); logger.println(magCalibration[2], 2);
    }
    ...
    */
    ...
    MS5637Reset();
    delay(100);
    logger.println("MS5637 pressure sensor reset...");
    // Read PROM data from MS5637 pressure sensor
    MS5637PromRead(Pcal);
    logger.println("PROM dta read:");
    logger.print("C0 = "); logger.println(Pcal[0]);
    unsigned char refCRC = Pcal[0] >> 12;
    logger.print("C1 = "); logger.println(Pcal[1]);
    logger.print("C2 = "); logger.println(Pcal[2]);
    logger.print("C3 = "); logger.println(Pcal[3]);
    logger.print("C4 = "); logger.println(Pcal[4]);
    logger.print("C5 = "); logger.println(Pcal[5]);
    logger.print("C6 = "); logger.println(Pcal[6]);

    nCRC = MS5637checkCRC(Pcal);  //calculate checksum to ensure integrity of MS5637 calibration data
    logger.print("Checksum = "); logger.print(nCRC); logger.print(" , should be "); logger.println(refCRC);
...
//      newData = false;  // reset newData flag
    ...
    if (delt_t > 500) { // update LCD once per half-second independent of read rate
      ...
      if (LoggerDebug) {
        logger.print("ax = "); logger.print((int)1000 * ax);
        logger.print(" ay = "); logger.print((int)1000 * ay);
        logger.print(" az = "); logger.print((int)1000 * az); logger.println(" mg");
        logger.print("gx = "); logger.print( gx, 2);
        logger.print(" gy = "); logger.print( gy, 2);
        logger.print(" gz = "); logger.print( gz, 2); logger.println(" deg/s");
        logger.print("mx = "); logger.print( (int)mx );
        logger.print(" my = "); logger.print( (int)my );
        logger.print(" mz = "); logger.print( (int)mz ); logger.println(" mG");
  
        logger.print("q0 = "); logger.print(q[0]);
        logger.print(" qx = "); logger.print(q[1]);
        logger.print(" qy = "); logger.print(q[2]);
        logger.print(" qz = "); logger.println(q[3]);
      }
      tempCount = readTempData();  // Read the gyro adc values
      temperature = ((float) tempCount) / 333.87 + 21.0; // Gyro chip temperature in degrees Centigrade
      // Print temperature in degrees Centigrade
      logger.print("Gyro temperature is ");  logger.print(temperature, 1);  logger.println(" degrees C"); // Print T values to tenths of s degree C
      ...
      lin_az = az - a33;
      if (LoggerDebug) {
        logger.print("Yaw, Pitch, Roll: ");
        logger.print(yaw, 2);
        logger.print(", ");
        logger.print(pitch, 2);
        logger.print(", ");
        logger.println(roll, 2);
  
        logger.print("Grav_x, Grav_y, Grav_z: ");
        logger.print(-a31 * 1000, 2);
        logger.print(", ");
        logger.print(-a32 * 1000, 2);
        logger.print(", ");
        logger.print(a33 * 1000, 2);  logger.println(" mg");
        logger.print("Lin_ax, Lin_ay, Lin_az: ");
        logger.print(lin_ax * 1000, 2);
        logger.print(", ");
        logger.print(lin_ay * 1000, 2);
        logger.print(", ");
        logger.print(lin_az * 1000, 2);  logger.println(" mg");
  
        logger.print("rate = "); logger.print((float)sumCount / sum, 2); logger.println(" Hz");
      }
  ...
  */
  ...
  void magcalMPU9250(float * dest1, float * dest2)
  {
    #ifdef HARD_CODED_CALIBRATION_VALUES
    ...
    logger.println("Mag Calibration: Wave device in a figure eight until done!");
    delay(4000);
    ...
    //    logger.println("mag x min/max:"); logger.println(mag_max[0]); logger.println(mag_min[0]);
    //    logger.println("mag y min/max:"); logger.println(mag_max[1]); logger.println(mag_min[1]);
    //    logger.println("mag z min/max:"); logger.println(mag_max[2]); logger.println(mag_min[2]);
    ...
    logger.println("Mag Calibration done!");
    #endif
  }
  ...
  */
  ...
  void MadgwickQuaternionUpdate(float ax, float ay, float az, float gx, float gy,
    ...
    norm =
      sqrtf(s1 * s1 + s2 * s2 + s3 * s3 + s4 * s4); // normalise step magnitude
    norm = 1.0f / norm;
    s1 *= norm;
    s2 *= norm;
    s3 *= norm;
