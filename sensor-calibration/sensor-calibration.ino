...
namespace
{
    ...
    void sensorUpdate(const sensors::SensorData &data)
    {
        ...
        if (data.imu_valid) {
            ...
            int64_t gyro_mag_squared = int64_t(data.gyro_data[0])*data.gyro_data[0] 
...
void setup()
{
    ...
    for (int i=0; !SerialUSB; i++) delay(100);
...
void loop()
{
    ...
    if (mode==MODE_GYRO_OFFSET) {
        ...
        Serial.println("[");
        for (const auto& res : results) Serial.println(String("  [") + res[0] + "," + res[1] + "," + res[2] + "],");
        Serial.println("]");
